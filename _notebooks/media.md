...
<div id="images" style="display: flex; flex-wrap: wrap; gap: 10px;">
    <script>
        ...
        function getRandomImages(imageFiles) {
            ...
            const getRandomSubset = (arr) => arr.sort(() => 0.5 - Math.random()).slice(0, 7);
            const combined = [...getRandomSubset(leftImages), ...getRandomSubset(rightImages), ...getRandomSubset(centerImages)];
...
<script type="module">
    ...
    async function fetchUser() {
        ...
        if (response.ok) {
            ...
            displayUsername.textContent = userInfo.name;
        } else {
    ...
    //Wait until images exist before adding drag listeners
    ...
    function postScore(username, finalScore) {
        ...
        .then(response => {
            if (response.ok) console.log('Score successfully saved!');
            else console.error('Failed to save score');
        })
